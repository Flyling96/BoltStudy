...
namespace Ludiq
{
	public sealed class GraphWindow : LudiqEditorWindow, ICanvasWindow
	{
		...
		[MenuItem("Window/Graph", priority = 0)]
		...
		private void OnEnable()
		{
			...
			{
				titleContent = new GUIContent(defaultTitle, LudiqGraphs.Icons.window?[IconSize.Small]);
		...
		private void OnContextChange()
		{
			...
			titleContent = new GUIContent(context?.windowTitle ?? defaultTitle, LudiqGraphs.Icons.window?[IconSize.Small]);
		...
		protected override void OnGUI()
		{
			...
			if (LudiqGraphs.instance == null || EditorApplication.isCompiling)
			{
		...
		public bool showSidebars =>
			reliablePosition.width >= LudiqGraphs.Configuration.graphSidebarThreshold.x &&
			reliablePosition.height >= LudiqGraphs.Configuration.graphSidebarThreshold.y &&
			context != null;
