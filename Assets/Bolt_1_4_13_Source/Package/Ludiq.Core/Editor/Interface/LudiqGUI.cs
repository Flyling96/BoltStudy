	...
	public static class LudiqGUI
	{
		...
		public static void Loader(Rect position)
		{
			...
			else
			{
				...
				{
					temporaryLoader = EditorTexture.Load(new AssemblyResourceProvider(Assembly.GetExecutingAssembly(), "Ludiq", "Resources"), "Loader/Loader.png", CreateTextureOptions.PixelPerfect, true);
				}
		...
		{
			Ludiq.Inspector.BeginBlock(titleMetadata, titlePosition, GUIContent.none);
			...
			if (Ludiq.Inspector.EndBlock(titleMetadata))
			{
		...
		{
			Ludiq.Inspector.BeginBlock(summaryMetadata, summaryPosition, GUIContent.none);
			...
			if (Ludiq.Inspector.EndBlock(summaryMetadata))
			{
		...
		)
		{
			...
			// hash.
			var activatorControlID = GUIUtility.GetControlID(HashUtility.GetHashCode(PopupHash, Ludiq.Inspector.currentBlock.metadata), FocusType.Keyboard, position);
